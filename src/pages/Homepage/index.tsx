...
export function Homepage() {
  ...
  const [filteredDisplacements, setFilteredDisplacements] = useState<
    DisplacementsData[]
  >([]);
  ...
  const onSearch = (value: string, typeInput: string) => {
    ...
    const dadosFiltrados = displacements.filter((entry) => {
    ...
    setFilteredDisplacements(dadosFiltrados);
  };
              ...
              <table className="table-auto min-w-full text-center border-collapse border border-[#B9B9B9] overflow-hidden bg-[#fff]">
                ...
                <tbody>
                  {filteredDisplacements.length !== 0 &&
                    filteredDisplacements.map((displacement) => (
